...
jane <- 5*2 # This evaluates 5 times 2
bozo = bob + jane
...
?glm
plot(glm(data[,3] ~ data[,2]))
abline(glm(data[,3] ~ data[,2])) # runs best fit line on whatever plot is being displayed
...
read.csv("C:/users/ranae/Desktop/data_course/code_examples/r_intro/iris.csv") # this prints it directly to the screen. we want to save it to an object
iris = read.csv("C:/users/ranae/Desktop/data_course/code_examples/r_intro/iris.csv") # this prints it directly to the screen. we want to save it to an object
...
class(iris)
iris[iris$Species == "virginica",] # pulls data for just species = virginica



# iris$Species $ allows for you to chose column
...
otus = read.csv("C:/Users/Ranae/Desktop/Data_Course/code_examples/R_intro/otu_table.csv", as.is = TRUE, stringsAsFactors = FALSE,
         check.names = FALSE)
metadata = read.csv("C:/Users/Ranae/Desktop/Data_Course/code_examples/R_intro/otu_mapping.csv", as.is = TRUE, stringsAsFactors = TRUE,
         check.names = FALSE)
...
sums = rowSums(TF)
maximum = max(sums)
#find is for command line, which is for R
...
# We can also easily create a heatmap to look at abundances broadly, we did not run this in class because it would take to long, and maybe lock up a laptop.
