...
#Operational Taxonomic Unit (OTU) <-- helpful when refering to a stretch of DNA found in the environment. 

# Check to see how even your sampling effort for each community is
barchart(rowSums(community_matrix)) #To compare this, move everything to the lowest common denominator, so that you can compare possibly oversampled 10 to sample 1
...
# This lets you know if your sample is big enough. This is what it should look like, if there is a linear/diagonal line, need to sample more. 
# The vertical line is the lowest common denominator, you want the curve to have flattened out before the vertical line. 
...
# Will randomly pick samples, in this case equal to min_depth

# How many species were found in each sample?
specnumber(community_matrix) #species number or richness.
...
# Calculate Shannon diversity for each sample, also called alpha diversity
...
plot(beta_disp)
#Tighter these triangles are, the more similar they are to each other. In this example, treatment 1 is more diverse than species 2 
#Treatment 2 is more closely related to each other compared to sample 1 
...
otu_table = read.delim("c:/Users/Ranae/Desktop/Data_Course/code_examples/Vegan_Example/vegan_example_otu_table.tsv", sep = "\t", row.names = 1)
metadata = read.delim("c:/Users/Ranae/Desktop/Data_Course/code_examples/Vegan_Example/vegan_example_metadata.tsv", sep = "\t", row.names = 1)
...
#Not quite flattened, so decide, is 2000 sufficient? Toss data? 

# Normalize the data by rarefying
set.seed(1) #Starts random number generator at same point (so that it can be duplicated on each machine)
...
?adonis()
adonis(otu_table ~ metadata$Ecosystem)
# This tells us that ecosystem is a good separating factor

adonis(otu_table ~ metadata$Latitude*metadata$Longitude)
# can't do aov because it is multidim, but if you could you could do a tukey test to see which is different. 